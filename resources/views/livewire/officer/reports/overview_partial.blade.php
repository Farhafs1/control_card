<div class="space-y-6 print:p-0">
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
        {{-- Use ?? [] to prevent error if key is missing --}}
        @forelse(($dataset['full_list'] ?? []) as $row)
            @php
                $label = $row['label'] ?? $row['category_name'] ?? 'N/A';
                $actual = (float) ($row['actual'] ?? 0);
                $perf  = (float) ($row['perf'] ?? 0);
                
                $cardColor = str_contains(strtolower($label), 'revenue') ? 'border-blue-600' : 'border-gray-800';
            ...
            <table class="w-full border-collapse min-w-[1100px]">
                <thead>
                    <tr class="bg-gray-800 text-white">
                        ...
                        <th class="px-4 py-4 text-right text-xs font-black uppercase tracking-wider bg-gray-700">Actual Q{{ $quarter }}</th>
                ...
                <tbody class="divide-y divide-gray-200">
                    @php $gApp = 0; $gAdd = 0; $gProv = 0; $gAct = 0; @endphp
                    ...
                    @forelse(($dataset['full_list'] ?? []) as $row)
                        ...
                        <tr class="hover:bg-gray-50 transition-colors">
                            ...
                            <td class="px-4 py-4 text-right text-sm font-mono text-green-700 font-bold bg-gray-50">
                                ₦{{ number_format($actual, 2) }}
                            </td>
                            <td class="px-4 py-4 text-right text-sm font-mono {{ $balance < 0 ? 'text-red-600' : 'text-gray-500' }}">
                                ₦{{ number_format($balance, 2) }}
                            </td>
                            <td class="px-4 py-4 text-right bg-green-50 border-l border-green-100">
                ...
                <tfoot class="bg-gray-900 text-white">
                    <tr>
                        <td class="px-4 py-5 text-xs font-black uppercase tracking-widest">Portfolio Totals</td>
                        ...
                        <td class="px-4 py-5 text-right bg-green-700">
                            <div class="text-[10px] text-green-200 font-bold uppercase">Portfolio Perf</div>
